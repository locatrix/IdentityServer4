...
namespace IdentityServer.UnitTests.Extensions
{
    public class IResourceStoreExtensionsTests
    {
        ...
        [Fact]
        public async Task GetAllEnabledResourcesAsync_on_duplicate_api_scopes_should_fail()
        {
            var store = new MockResourceStore()
            {
            ...
            Func<Task> a = () => store.GetAllEnabledResourcesAsync();
            (await a.Should().ThrowAsync<Exception>()).And.Message.ToLowerInvariant().Should().Contain("duplicate").And.Contain("api scopes");
        }
        ...
        [Fact]
        public async Task FindResourcesByScopeAsync_on_duplicate_api_scopes_should_fail()
        {
            var store = new MockResourceStore()
            {
                ApiResources = { new ApiResource("A"), new ApiResource("A") }
